# Clean Process Frames
# Timothy Berrill June 2017

# This is a much cleaned up version of the framing classifier data processing
# program. It takes the Media Frames Corpus json files and processes the data
# into a much better (more workable / readable) format. This includes extracting
# the correct text of the annotations, matching them with sentences and creating
# full "AnnotatedArticle" objects which include all of the relevant information
# necessary to easily utilize the information.
...
# Colors class is just used to change the text color of print outs, making them
# easier to read. This is done by print(Colors.COLOR + "string" + Colors.RESET)
class Colors:
...
class Annotation(object):
    ...
    def __init__(self, start, end, code, text, frame, text_with_punctuation):
        if text:
...
def match_annotations_to_sentences(articles, verbose, error_verbose):
    ...
    for x0, article in enumerate(articles):
        for x1, annotation in enumerate(article.annotations):
            ...
            for x2, sentence in enumerate(article.sentences):
                if not is_found:
                    ...
                    if (annotation.text in sentence_no_punctuation) and (start <= start_buffer):
                        repeat_count += 1
                        sentence.add_frame(annotation.code, annotation.frame)
                        is_found = True
                        if verbose:
                            print "Sentences: " + sentence.text 
                            print "Added Frame: " + str(annotation.frame) + Colors.RESET
                else:
                    break
            ...
            if repeat_count >= 1:
                if error_verbose:
                    print "Repeated annotation: " + Colors.BLUE + str(repeat_count) + Colors.RESET
                    print "Annotation text: " + annotation.text
...
# present in the json.
def create_articles(articles, information_json, held_out):
    total_articles = 0
    empty_count = 0
    ...
    for x8, key in enumerate(information_json.keys()):
            ...
            for annotation in all_list:
                ...
                if len(temp_text) > temp_start:
                    ...
                    temp_annotation = Annotation(temp_start, temp_end, temp_code,
                                                 temp_quote_no_punctuation, temp_type, temp_quote)
        ...
        if (is_empty):
            ...
            held_out.append(temp_article)
        count += 1
    print "Empty Frames: " + str(empty_count) + " / " + str(total_articles) + "   " + str(empty_count/total_articles*100) + "%"
    return
...
def main(load):
    if (load):
        ...
        for information in files:
            articles = []
            held_out = []
            create_articles(articles, information, held_out)
            error = match_annotations_to_sentences(articles, False, False)
            annotated_articles.append((names[count], articles, held_out))
        ...
        annotations_f = open('annotations.p', 'wb')
        pickle.dump(annotated_articles, annotations_f)
        annotations_f.close()