...
class FeatureExtractor(object):
    ...
    def executeExtractor(self):
        ...
        print '45 training + 5 testing \n'
        size = int(len(featuresets) * 0.1)
        train_set, test_set = featuresets[size:], featuresets[:size]
        nb_classifier = nltk.NaiveBayesClassifier.train(train_set)
        nb_classifier.show_most_informative_features(100)
        self.evaluate_model(nb_classifier,test_set)
        saveModel(nb_classifier,'nb_filtered_50')

        # Train up a classifier.
        print '36 training + 4 testing \n'
        size = int(len(featuresets) * 0.8)
        train_set, test_set = featuresets[int(size*0.1):size], featuresets[:int(size*0.1)]
        nb_classifier = nltk.NaiveBayesClassifier.train(train_set)
        nb_classifier.show_most_informative_features(100)
        self.evaluate_model(nb_classifier,test_set)
        saveModel(nb_classifier,'nb_filtered_40')
        print '27 training + 3 testing \n'
        size = int(len(featuresets) * 0.6)
        train_set, test_set = featuresets[int(size*0.1):size], featuresets[:int(size*0.1)]
        nb_classifier = nltk.NaiveBayesClassifier.train(train_set)
        nb_classifier.show_most_informative_features(100)
        self.evaluate_model(nb_classifier,test_set)
        saveModel(nb_classifier,'nb_filtered_30')
        print '18 training + 2 testing \n'
        size = int(len(featuresets) * 0.4)
        train_set, test_set = featuresets[int(size*0.1):size], featuresets[:int(size*0.1)]
        nb_classifier = nltk.NaiveBayesClassifier.train(train_set)
        nb_classifier.show_most_informative_features(100)
        self.evaluate_model(nb_classifier,test_set)
        saveModel(nb_classifier,'nb_filtered_20')
    ...
    def evaluate_model(self, classifier, test_set):
        ...
        print 'precision:', nltk.metrics.precision(refsets['True'], testsets['True'])
        print 'recall:', nltk.metrics.recall(refsets['True'], testsets['True'])
        print 'F-measure:', nltk.metrics.f_measure(refsets['True'], testsets['True'])
