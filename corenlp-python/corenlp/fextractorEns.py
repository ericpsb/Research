...
class FeatureExtractorEns(object):
    """
    NLP Tool to train and test Political Framing Words Classifiers
    """
  ...
  #              X = self.transformer_.transform(X)
   #             return LinearSVC.predict(self, X)
    ...
    def corpusFromDB(self, doc_num):
        ...
        c.execute("SELECT  doc_id, doc_html, sum(valid) as tot_valid FROM Documents natural join Annotations WHERE doc_id >3 AND doc_id<>192 group by doc_id having tot_valid >= 3 order by doc_id desc LIMIT %s "%(doc_num,))#a_id > 125")
    ...
    def TFIDF(self, word,document):
        ...
        return (self.tfidf_bins[document].get(word, 8)) / 8.0 # use this so punct get the max tfidf bin
    ...
    def generateFeatures(self, outer_index, words,deps):
        ...
        if feat_imagery:
            ...
            fnames.extend(['abstractness ', 'abstractness: +1 ', 'abstractness: -1 ',
                           'abstractness: +2 ', 'abstractness: -2 '])
            ...
            try:
                features['abstractness ']=self.abst_dict[stems[index]]
            except KeyError:
                features['abstractness ']='null'
                    
            
            self.plus_minus_features_dict('abstractness', 1, stems, self.dsp_dict, index, 
                    features)
            self.plus_minus_features_dict('abstractness', 2, stems, self.dsp_dict, index, 
                    features)
    ...
    def runEnsemble(self):
        ...
        classifiers= [
                ...
                (Perceptron(n_iter=42, shuffle=True), "Perceptron 42"),
                #(KNeighborsClassifier(n_neighbors=10), "k Nearest Neighbors 10"),
                ...
                #(MultinomialNB(alpha=.01), "Multinomial Naive Bayes - alpha .01"),
                #(MultinomialNB(alpha=.05), "Multinomial Naive Bayes - alpha .05")
                ]
        #f = open('theoretical only distributions.csv', 'w')
        for clf,name in classifiers:
            t0=time()
            #if self.rounds>1:
            #    clf=loadModel(norm_ation_thresh,name)
         #   try:
            #clf.fit(self.X_train, self.y_train)
...
def saveModel(classifier, thresh,type='nb'):
    ...
    f='classifiers/'+type+'_classifier'+str(thresh)+'.pickle'
    joblib.dump(classifier, f)
    #f.close()

def loadModel(thresh, type='nb'):
    """
