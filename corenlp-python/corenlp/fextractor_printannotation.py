  ...
  #              X = self.transformer_.transform(X)
   #             return LinearSVC.predict(self, X)
	...
	"""
        ...
        c=db.cursor()
        c.execute("SELECT  DISTINCT(doc_id),doc_html, sum(valid) as tot_valid FROM Documents natural join Annotations WHERE doc_id != 1 group by doc_id having tot_valid >= 3 order by doc_id desc LIMIT %s "%(doc_num,))#a_id > 125")
            ...
            for i in range(len(self.coreParsed)):
                ...
                for windex in range(len(words)):
                    ...
                    for a in range(len(self.start_indices)):
                        ...
                        '''
                    train_set.append(f1)
                    targets.append(1 if float(ation_aggregate) / len(self.start_indices) >= float(1/3) else 0)
                    f_counter+=1
