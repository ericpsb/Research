...
class test_spider(scrapy.Spider):
    name = "scraper"
    start_urls = ['https://autismandoughtisms.wordpress.com/']
    custom_settings = {
        'RETRY_TIMES': 10,
    ...
    def parse_post(self, response):
      ...
      try:
        title = response.xpath('//h2[@class="entry-title"]/a/text()').extract()[0]
      except:
        title =""
      ...
      try:
        ...
        for href in response.xpath('//div[@class="entry-content"]//*[not(descendant::div[@class="wpcnt"]) and not(ancestor-or-self::div[@class="wpcnt"]) and not(descendant::div[@id="jp-post-flair"]) and not(ancestor-or-self::div[@id="jp-post-flair"])]/@href'):
          ...
          if 'https://autismandoughtisms.wordpress.com/' not in extracted:
