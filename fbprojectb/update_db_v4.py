...
def main():
    ...
    if (check < len(lines)):
    	...
    	logging.info("Number of new users: {}. Including blank lines".format(len(remaining)))
        print len(remaining)
        ...
        for line in remaining:
            ...
            if (token == '\n'):
                with open('check_db.txt', 'w') as filetemp:
                    ...
                    logging.info("check_db.txt is updated for empty line. Now after line {}".format(check))
                continue
    	    ...
    	    if ('error' in profile):
    	        ...
    	        with open('check_db.txt', 'w') as filetemp:
    	            filetemp.write(str(check))
    	            logging.info("check_db.txt is updated for an error in access token. Now after line {}".format(check))
    	        continue
            ...
            if existing == None:
                ...
                logging.info("Inserted new user {} into User".format(user_name))
            else:
            ...
            update_db(token,activity_url,profile,user_name,user_id,user,people,events,friends,mutual,taggable_friends,likes,feeds,interactions,to_update_friends)
            ...
            with open('check_db.txt', 'w') as filetwo:
            	...
            	logging.info("check_db.txt is updated for successfully collecting a user's data.")
...
def update_db(token,activity_url,profile,user_name,user_id,user,people,events,friends,mutual,taggable_friends,likes,feeds,interactions,to_update_friends):
	...
	logging.info("Error in getting {}\'s timeline data: {}".format(user_name,activities))
    ...
    if 'friends' in activities:
        ...
        for friend in friends_list:
            ...
            if friends.find_one({"id": person['id']}) == None:
                ...
                friends.update({"id": person['id']},{"$addToSet": {"friend_of": {"name": user_name, "id": user_id}}})
